...
import ProductsMarkup from './ProductsMarkup';
...
const Products = props => {
  ...
  // range slider material --
  // ------------------ OPTIONS ------------------------ //
  // (Use props._VALUE_ in this section if needed)
  const icon = 'circle';
  const minBoundary = 0;
  const maxBoundary = 25000;
  const min_initVal = 0;
  const max_initVal = 25000;
  ...
  const min_getSlider = () => {
    return (
      <Animated.View
        ...
        <View style={s.circle}>
          <View style={s.icon}>
            <FontAwesome name={icon} size={30} color={colorHighlight} />
          </View>
        </View>
      </Animated.View>
    );
  ...
  useEffect(() => {
    ...
    let maxVal = max_animState.displayVal;
    let minVal = min_animState.displayVal;

    dispatch(getAllProducts(keyword, 1, [minVal, maxVal], category, ratings));
  }, [
    dispatch,
    error,
    ratings,
  ...
  return (
    <ProductsMarkup
      {...props}
      min_animState={min_animState}
      sliderHeight={sliderHeight}
      manualOffsetBetweenSlider={manualOffsetBetweenSlider}
      colorHighlight={colorHighlight}
      max_animState={max_animState}
      min_getLine={min_getLine}
      max_getLine={max_getLine}
      min_getSlider={min_getSlider}
